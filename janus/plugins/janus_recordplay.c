...
void janus_recordplay_create_session(janus_plugin_session *handle, int *error) {
	...
	session->video_keyframe_interval = 3000; 	/* 15 seconds by default */
...
void janus_recordplay_setup_media(janus_plugin_session *handle) {
	...
	if(!session->recorder) {
		GError *error = NULL;
		janus_refcount_increase(&session->ref);
		GThread* record_playout_thread = NULL;
		record_playout_thread = g_thread_try_new("recordplay playout thread", &janus_recordplay_playout_thread, session, &error);
		if(error != NULL) {
			janus_refcount_decrease(&session->ref);

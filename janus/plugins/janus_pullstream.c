/*! \file   janus_pullstream.c
 * \author Lorenzo Miniero <lorenzo@meetecho.com>
 * \copyright GNU General Public License v3
 * \brief  Janus pullstream plugin
 * \details Check the \ref pullstream for more details.
 *
 * \ingroup plugins
 * \ref plugins
 *
 * \page pullstream pullstream plugin documentation
 * This is a pullstream plugin for Janus, allowing WebRTC peers
 ...
 *
 * -# on-demand pullstream of pre-recorded media files (different
 * pullstream context for each peer);
 * -# live pullstream of pre-recorded media files (shared pullstream
 * context for all peers attached to the stream);
 * -# live pullstream of media generated by another tool (shared
 * pullstream context for all peers attached to the stream).
 *
 ...
 * added  the possibility to also add a datachannel track to an RTP pullstream
 ...
 * is a radio pullstream channel). When using this feature, though, beware
 ...
 * A pre-filled configuration file is provided in \c conf/janus.plugin.pullstream.cfg
...
type = rtp|live|ondemand|rtsp
       ...
       live = local file streamed live to multiple viewers
              (multiple viewers = same pullstream context)
       ondemand = local file streamed on-demand to a single listener
                  (multiple viewers = different pullstream contexts)
...
threads = number of threads to assist with the relaying part, which can help
	...
	in the pullstream plugin to slow down and fail to catch up (default=0)
...
\endverbatim
 *
 * \section streamapi pullstream API
 *
 * The pullstream API supports several requests, some of which are
 ...
 *
 * \subsection pullstreamsync Synchronous requests
 *
 * To list the available pullstream mountpoints (both those created via
...
\endverbatim
 *
 * \subsection pullstreamasync Asynchronous requests
 *
 ...
 * establishment to succeed. As soon as that happens, the pullstream plugin
 ...
 *
 * Another interesting feature in the pullstream plugin is the so-called
...
#include "../utils.h"
#include "../ip-utils.h"
#include "../rtp_rtmp/rtmp_client_live_play.h"
#include "../rtp_rtmp/flv_demuxer.h"
#include "../rtp_rtmp/aac_decoder.h"
#include "../rtp_rtmp/opus_encoder.h"
#include "../rtp_rtmp/rtp_muxer.h"

#define HAVE_LIBCURL
//#define USE_PCM 1
//#define RESAMPLE_PCM 1

/* Plugin information */
#define JANUS_PULLSTREAM_VERSION			8
#define JANUS_PULLSTREAM_VERSION_STRING	    "0.0.8"
#define JANUS_PULLSTREAM_DESCRIPTION		"This is a pullstream plugin for Janus, allowing WebRTC peers to watch/listen to pre-recorded files or media generated by gstreamer."
#define JANUS_PULLSTREAM_NAME			    "JANUS Pullstream plugin"
#define JANUS_PULLSTREAM_AUTHOR			    "Meetecho s.r.l."
#define JANUS_PULLSTREAM_PACKAGE			"janus.plugin.pullstream"

/* Plugin methods */
janus_plugin *create(void);
int janus_pullstream_init(janus_callbacks *callback, const char *config_path);
void janus_pullstream_destroy(void);
int janus_pullstream_get_api_compatibility(void);
int janus_pullstream_get_version(void);
const char *janus_pullstream_get_version_string(void);
const char *janus_pullstream_get_description(void);
const char *janus_pullstream_get_name(void);
const char *janus_pullstream_get_author(void);
const char *janus_pullstream_get_package(void);
void janus_pullstream_create_session(janus_plugin_session *handle, int *error);
struct janus_plugin_result *janus_pullstream_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep);
void janus_pullstream_setup_media(janus_plugin_session *handle);
void janus_pullstream_incoming_rtp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_pullstream_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len);
void janus_pullstream_hangup_media(janus_plugin_session *handle);
void janus_pullstream_destroy_session(janus_plugin_session *handle, int *error);
json_t *janus_pullstream_query_session(janus_plugin_session *handle);
static int janus_pullstream_get_fd_port(int fd);

static int rtmp_client_live_play_callback(void* param, const unsigned char* data, size_t bytes, uint32_t timestamp, int type);
static void flv_demuxer_video_audio_callback(void* param, int codec, const unsigned char* data, size_t bytes, uint32_t pts, uint32_t dts, int flags);
static void aac_decoder_callback(void* param, const unsigned char* pdata, int bytes, uint32_t timestamp);
static void opus_encoder_callback(void* param, const unsigned char* pdata, int bytes, uint32_t timestamp);
static void rtp_muxer_packet_callback (void* param, const void *packet, int bytes, RTP_TYPE type);


/* Plugin setup */
static janus_plugin janus_pullstream_plugin =
	JANUS_PLUGIN_INIT (
		.init = janus_pullstream_init,
		.destroy = janus_pullstream_destroy,

		.get_api_compatibility = janus_pullstream_get_api_compatibility,
		.get_version = janus_pullstream_get_version,
		.get_version_string = janus_pullstream_get_version_string,
		.get_description = janus_pullstream_get_description,
		.get_name = janus_pullstream_get_name,
		.get_author = janus_pullstream_get_author,
		.get_package = janus_pullstream_get_package,

		.create_session = janus_pullstream_create_session,
		.handle_message = janus_pullstream_handle_message,
		.setup_media = janus_pullstream_setup_media,
		.incoming_rtp = janus_pullstream_incoming_rtp,
		.incoming_rtcp = janus_pullstream_incoming_rtcp,
		.hangup_media = janus_pullstream_hangup_media,
		.destroy_session = janus_pullstream_destroy_session,
		.query_session = janus_pullstream_query_session,
	);

/* Plugin creator */
janus_plugin *create(void) {
	JANUS_LOG(LOG_VERB, "%s created!\n", JANUS_PULLSTREAM_NAME);
	return &janus_pullstream_plugin;
}
...
static void *janus_pullstream_handler(void *data);

static void *janus_pullstream_ondemand_thread(void *data);
static void *janus_pullstream_filesource_thread(void *data);
static void janus_pullstream_relay_rtp_packet(gpointer data, gpointer user_data);
static void janus_pullstream_relay_rtcp_packet(gpointer data, gpointer user_data);
static void *janus_pullstream_relay_thread(void *data);
static void *janus_pullstream_rtmpsource_thread(void *data);
static void janus_pullstream_hangup_media_internal(janus_plugin_session *handle);

typedef enum janus_pullstream_type {
	janus_pullstream_type_none = 0,
	janus_pullstream_type_live,
	janus_pullstream_type_on_demand,
} janus_pullstream_type;

typedef enum janus_pullstream_source {
	janus_pullstream_source_none = 0,
	janus_pullstream_source_file,
	janus_pullstream_source_rtp,
	janus_pullstream_source_rtmp,
} janus_pullstream_source;

typedef struct janus_pullstream_rtp_keyframe {
	gboolean enabled;
	...
	guint32 temp_ts;
	janus_mutex mutex;
} janus_pullstream_rtp_keyframe;

typedef struct janus_pullstream_rtp_relay_packet {
...
} janus_pullstream_rtp_relay_packet;
static janus_pullstream_rtp_relay_packet exit_packet;
static void janus_pullstream_rtp_relay_packet_free(janus_pullstream_rtp_relay_packet *pkt) {
...
#ifdef HAVE_LIBCURL
typedef struct janus_pullstream_buffer {
	char *buffer;
	size_t size;
} janus_pullstream_buffer;
#endif

typedef struct janus_pullstream_rtp_source {
	...
	gboolean rtsp;
	CURL *curl;
	janus_pullstream_buffer *curldata;
	char *rtsp_url;
...
#endif
	janus_pullstream_rtp_keyframe keyframe;
	gboolean buffermsg;
	int rtp_collision;
	void *last_msg;
...
} janus_pullstream_rtp_source;

typedef struct janus_pullstream_rtmp_source {
	gint audio_port, remote_audio_port;
	gint audio_rtcp_port, remote_audio_rtcp_port;
	in_addr_t audio_mcast;
	gint video_port[3], remote_video_port;
	...
	srtp_t srtp_ctx;
	srtp_policy_t srtp_policy;
} janus_pullstream_rtmp_source;


typedef struct janus_pullstream_file_source {
	char *filename;
} janus_pullstream_file_source;
...
typedef struct janus_pullstream_codecs {
	gint audio_pt;
	char *audio_rtpmap;
	char *audio_fmtp;
	...
	char *video_rtpmap;
	char *video_fmtp;
} janus_pullstream_codecs;

typedef struct janus_pullstream_mountpoint {
	guint64 id;
	char *name;
	char *listener;
	char *description;
	...
	janus_pullstream_type pullstream_type;
	janus_pullstream_source pullstream_source;
	void *source;	/* Can differ according to the source type */
	GDestroyNotify source_destroy;
	janus_pullstream_codecs codecs;
	...
	janus_mutex mutex;
	janus_refcount ref;
	struct rtmp_client_live_play_context_t* rtmp_client;
	struct flv_demuxer_video_audio_context_t* flv_demuxer;
	struct aac_decoder_context_t* aac_decoder;
	struct opus_encode_context_t* opus_encoder;
	struct rtp_muxer_context_t*   rtp_muxer;

} janus_pullstream_mountpoint;
static GHashTable *mountpoints;
static janus_mutex mountpoints_mutex;
static char *admin_key = NULL;

typedef struct janus_pullstream_helper {
	janus_pullstream_mountpoint *mp;
	guint id;
	GThread *thread;
	int num_viewers;
	GList *viewers;
	...
	volatile gint destroyed;
	janus_mutex mutex;
} janus_pullstream_helper;
static void *janus_pullstream_helper_thread(void *data);
static void janus_pullstream_helper_rtprtcp_packet(gpointer data, gpointer user_data);
static void rtp_send(janus_pullstream_mountpoint* mountpoint, const unsigned char* pdata, int bytes, RTP_TYPE type);

/* Helper to create an RTP live source (e.g., from gstreamer/ffmpeg/vlc/etc.) */
janus_pullstream_mountpoint *janus_pullstream_create_rtp_source(
...
janus_pullstream_mountpoint *janus_pullstream_create_file_source(
...
janus_pullstream_mountpoint *janus_pullstream_create_rtsp_source(
		uint64_t id, char *name, char *desc,
...
typedef struct janus_pullstream_message {
	janus_plugin_session *handle;
	char *transaction;
	json_t *message;
	json_t *jsep;
} janus_pullstream_message;
static GAsyncQueue *messages = NULL;
static janus_pullstream_message exit_message;

typedef struct janus_pullstream_session {
	janus_plugin_session *handle;
	janus_pullstream_mountpoint *mountpoint;
	gint64 sdp_sessid;
	gint64 sdp_version;
	gboolean started;
	gboolean paused;
	...
	volatile gint destroyed;
	janus_refcount ref;
} janus_pullstream_session;
static GHashTable *sessions;
static janus_mutex sessions_mutex = JANUS_MUTEX_INITIALIZER;

static void janus_pullstream_session_destroy(janus_pullstream_session *session) {
	if (session && g_atomic_int_compare_and_exchange(&session->destroyed, 0, 1))
	{
		if (session->mountpoint != NULL)
		{
			g_atomic_int_compare_and_exchange(&session->mountpoint->destroyed, 0, 1);
		}
		janus_refcount_decrease(&session->ref);
	}
}

static void janus_pullstream_session_free(const janus_refcount *session_ref) {
	janus_pullstream_session *session = janus_refcount_containerof(session_ref, janus_pullstream_session, ref);
...
static void janus_pullstream_mountpoint_destroy(janus_pullstream_mountpoint *mountpoint) {
	if(!mountpoint)
		return;

	/*if(!g_atomic_int_compare_and_exchange(&mountpoint->destroyed, 0, 1))
		return;
	在session释放过的释放 已经做了这步操作了	
	*/
	/* If this is an RTP source, interrupt the poll */
	if(mountpoint->pullstream_source == janus_pullstream_source_rtp) {
		janus_pullstream_rtp_source *source = mountpoint->source;
	...
	if (mountpoint->pullstream_source == janus_pullstream_source_rtmp) {
		janus_pullstream_rtmp_source *source = mountpoint->source;
		/*if (source != NULL && source->pipefd[1] > 0) {
			int code = 1;
			ssize_t res = 0;
			do {
	...
	if(mountpoint->helper_threads > 0) {
		...
		while(l) {
			janus_pullstream_helper *ht = (janus_pullstream_helper *)l->data;
...
static void janus_pullstream_mountpoint_free(const janus_refcount *mp_ref) {
	janus_pullstream_mountpoint *mp = janus_refcount_containerof(mp_ref, janus_pullstream_mountpoint, ref);
...
static void janus_pullstream_message_free(janus_pullstream_message *msg) {
	if(!msg || msg == &exit_message)
		return;

	if(msg->handle && msg->handle->plugin_handle) {
		janus_pullstream_session *session = (janus_pullstream_session *)msg->handle->plugin_handle;
		janus_refcount_decrease(&session->ref);
	}
	msg->handle = NULL;
...
static void janus_pullstream_rtcp_pli_send(janus_pullstream_rtp_source *source) {
...
static void janus_pullstream_rtcp_remb_send(janus_pullstream_rtp_source *source) {
	if(source == NULL || source->video_rtcp_fd < 0 || source->video_rtcp_addr.sa_family == 0)
		return;
...
/* Error codes */
#define JANUS_PULLSTREAM_ERROR_NO_MESSAGE			450
#define JANUS_PULLSTREAM_ERROR_INVALID_JSON			451
#define JANUS_PULLSTREAM_ERROR_INVALID_REQUEST		452
#define JANUS_PULLSTREAM_ERROR_MISSING_ELEMENT		453
#define JANUS_PULLSTREAM_ERROR_INVALID_ELEMENT		454
#define JANUS_PULLSTREAM_ERROR_NO_SUCH_MOUNTPOINT	455
#define JANUS_PULLSTREAM_ERROR_CANT_CREATE			456
#define JANUS_PULLSTREAM_ERROR_UNAUTHORIZED			457
#define JANUS_PULLSTREAM_ERROR_CANT_SWITCH			458
#define JANUS_PULLSTREAM_ERROR_CANT_RECORD			459
#define JANUS_PULLSTREAM_ERROR_UNKNOWN_ERROR			470

janus_pullstream_mountpoint *janus_pullstream_create_rtmp_source(uint64_t id, char *name, char *desc,
	char *url, gboolean doaudio, char *artpmap, char *afmtp,
	gboolean dovideo, char *vrtpmap, char *vfmtp,
	const janus_network_address *iface,
	gboolean error_on_failure);
...
int janus_pullstream_init(janus_callbacks *callback, const char *config_path) {
	...
	/* Read configuration */
	char filename[255];
	g_snprintf(filename, 255, "%s/%s.cfg", config_path, JANUS_PULLSTREAM_PACKAGE);
	...
	mountpoints = g_hash_table_new_full(g_int64_hash, g_int64_equal, (GDestroyNotify)g_free, (GDestroyNotify)janus_pullstream_mountpoint_destroy);
		...
		while(cl != NULL) {
			...
			if(!strcasecmp(type->value, "rtp")) 
			{
				...
				if(id == NULL || id->value == NULL) {
					...
					janus_pullstream_mountpoint *mp = g_hash_table_lookup(mountpoints, &mpid);
				...
				janus_pullstream_mountpoint *mp = NULL;
				if((mp = janus_pullstream_create_rtp_source(
			...
			else if(!strcasecmp(type->value, "live")) 
			{
				...
				if(!doaudio || dovideo) {
					JANUS_LOG(LOG_ERR, "Can't add 'live' stream '%s', we only support audio file pullstream right now...\n", cat->name);
					cl = cl->next;
					continue;
				}
				...
				janus_pullstream_mountpoint *mp = NULL;
				if((mp = janus_pullstream_create_file_source(
						(id && id->value) ? g_ascii_strtoull(id->value, 0, 10) : 0,
						(char *)cat->name,
						desc ? (char *)desc->value : NULL,
						(char *)file->value,
			...
			else if(!strcasecmp(type->value, "ondemand")) 
			{
				...
				/* TODO We should support something more than raw a-Law and mu-Law streams... */
				if(!doaudio || dovideo) {
					JANUS_LOG(LOG_ERR, "Can't add 'ondemand' stream '%s', we only support audio file pullstream right now...\n", cat->name);
					cl = cl->next;
					continue;
				}
				if(!strstr(file->value, ".alaw") && !strstr(file->value, ".mulaw")) {
			...
			else if(!strcasecmp(type->value, "rtsp")) 
			{
				...
				gboolean doaudio = audio && audio->value && janus_is_true(audio->value);
				gboolean dovideo = video && video->value && janus_is_true(video->value);
				gboolean error_on_failure = FALSE;
				...
				janus_pullstream_mountpoint *mp = NULL;
				if((mp = janus_pullstream_create_rtsp_source(
						(id && id->value) ? g_ascii_strtoull(id->value, 0, 10) : 0,
						(char *)cat->name,
						desc ? (char *)desc->value : NULL,
						(char *)file->value,
			...
			else if (!strcasecmp(type->value,"rtmp"))
			{
				/*janus_config_item *id = janus_config_get_item(cat, "id");
				janus_config_item *desc = janus_config_get_item(cat, "description");
				janus_config_item *priv = janus_config_get_item(cat, "is_private");
				...
				janus_network_address iface_value;
				if (file == NULL || file->value == NULL) {
					JANUS_LOG(LOG_ERR, "Can't add 'rtmp' stream '%s', missing mandatory information...\n", cat->name);
					cl = cl->next;
					continue;
				}
				gboolean is_private = priv && priv->value && janus_is_true(priv->value);
				...
				if ((doaudio || dovideo) && iface && iface->value) {
					if (!ifas) {
						JANUS_LOG(LOG_ERR, "Skipping 'rtmp' stream '%s', it relies on network configuration but network device information is unavailable...\n", cat->name);
						cl = cl->next;
						continue;
					}
					if (janus_network_lookup_interface(ifas, iface->value, &iface_value) != 0) {
						JANUS_LOG(LOG_ERR, "Can't add 'rtmp' stream '%s', invalid network interface configuration for stream...\n", cat->name);
						cl = cl->next;
						continue;
					}
				}
				...
				janus_pullstream_mountpoint *mp = NULL;
				if ((mp = janus_pullstream_create_rtmp_source(
					(id && id->value) ? g_ascii_strtoull(id->value, 0, 10) : 0,
					(char *)cat->name,
					desc ? (char *)desc->value : NULL,
					(char *)file->value,
					...
					iface && iface->value ? &iface_value : NULL,
					error_on_failure)) == NULL) {
					JANUS_LOG(LOG_ERR, "Error creating 'rtmp' stream '%s'...\n", cat->name);
					cl = cl->next;
					continue;
				}
				mp->is_private = is_private;
	...
	while(g_hash_table_iter_next(&iter, NULL, &value)) {
		janus_pullstream_mountpoint *mp = value;
		JANUS_LOG(LOG_VERB, "  ::: [%"SCNu64"][%s] %s (%s, %s, %s, pin: %s)\n", mp->id, mp->name, mp->description,
			mp->pullstream_type == janus_pullstream_type_live ? "live" : "on demand",
			mp->pullstream_source == janus_pullstream_source_rtp ? "RTP source" : "file source",
			mp->is_private ? "private" : "public",
			mp->pin ? mp->pin : "no pin");
	}
	janus_mutex_unlock(&mountpoints_mutex);

	sessions = g_hash_table_new_full(NULL, NULL, NULL, (GDestroyNotify)janus_pullstream_session_destroy);
	messages = g_async_queue_new_full((GDestroyNotify) janus_pullstream_message_free);
	/* This is the callback we'll need to invoke to contact the Janus core */
	gateway = callback;

	/* Launch the thread that will handle incoming messages */
	GError *error = NULL;
	handler_thread = g_thread_try_new("pullstream handler", janus_pullstream_handler, NULL, &error);
	if(error != NULL) {
		g_atomic_int_set(&initialized, 0);
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the pullstream handler thread...\n", error->code, error->message ? error->message : "??");
...
int janus_pullstream_get_version(void) {
	return JANUS_PULLSTREAM_VERSION;
}

const char *janus_pullstream_get_version_string(void) {
	return JANUS_PULLSTREAM_VERSION_STRING;
}

const char *janus_pullstream_get_description(void) {
	return JANUS_PULLSTREAM_DESCRIPTION;
}

const char *janus_pullstream_get_name(void) {
	return JANUS_PULLSTREAM_NAME;
}

const char *janus_pullstream_get_author(void) {
	return JANUS_PULLSTREAM_AUTHOR;
}

const char *janus_pullstream_get_package(void) {
	return JANUS_PULLSTREAM_PACKAGE;
}

static janus_pullstream_session *janus_pullstream_lookup_session(janus_plugin_session *handle) {
	janus_pullstream_session *session = NULL;
	if(g_hash_table_contains(sessions, handle)) {
		session = (janus_pullstream_session *)handle->plugin_handle;
	}
	return session;
}
...
void janus_pullstream_create_session(janus_plugin_session *handle, int *error) {
	...
	janus_pullstream_session *session = g_malloc0(sizeof(janus_pullstream_session));
	...
	janus_refcount_init(&session->ref, janus_pullstream_session_free);
	janus_mutex_lock(&sessions_mutex);
...
void janus_pullstream_destroy_session(janus_plugin_session *handle, int *error) {
	...
	janus_mutex_lock(&sessions_mutex);
	janus_pullstream_session *session = janus_pullstream_lookup_session(handle);
	if(!session) {
		janus_mutex_unlock(&sessions_mutex);
	...
	JANUS_LOG(LOG_VERB, "Removing pullstream session...\n");
	janus_pullstream_hangup_media_internal(handle);
...
json_t *janus_pullstream_query_session(janus_plugin_session *handle) {
	...
	janus_pullstream_mountpoint *mp = session->mountpoint;
	...
	if(mp) {
		...
		if(mp->pullstream_source == janus_pullstream_source_rtp) {
			janus_pullstream_rtp_source *source = mp->source;
			if(source->simulcast) {
		...
		if (mp->pullstream_source == janus_pullstream_source_rtmp) {
			janus_pullstream_rtmp_source *source = mp->source;
			if (source->simulcast) {
				json_t *simulcast = json_object();
				json_object_set_new(simulcast, "substream", json_integer(session->substream));
				json_object_set_new(simulcast, "substream-target", json_integer(session->substream_target));
...
struct janus_plugin_result *janus_pullstream_handle_message(janus_plugin_session *handle, char *transaction, json_t *message, json_t *jsep) {
	...
	JANUS_LOG(LOG_INFO, "janus_pullstream_handle_message       pullstream ++++++++\n");
	...
	if(!session) {
		janus_mutex_unlock(&sessions_mutex);
		JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
		error_code = JANUS_PULLSTREAM_ERROR_UNKNOWN_ERROR;
	...
	if(message == NULL) {
		...
		error_code = JANUS_PULLSTREAM_ERROR_NO_MESSAGE;
		g_snprintf(error_cause, 512, "%s", "No message??");
		goto plugin_response;
	}
	if(!json_is_object(root)) {
		JANUS_LOG(LOG_ERR, "JSON error: not an object\n");
		error_code = JANUS_PULLSTREAM_ERROR_INVALID_JSON;
	...
	JANUS_VALIDATE_JSON_OBJECT(root, request_parameters,
		...
		JANUS_PULLSTREAM_ERROR_MISSING_ELEMENT, JANUS_PULLSTREAM_ERROR_INVALID_ELEMENT);
	if(error_code != 0)
		goto plugin_response;
	...
	JANUS_LOG(LOG_INFO, "janus_pullstream_handle_message request_text=%s\n",request_text);
	if(!strcasecmp(request_text, "list")) {
		...
		while(g_hash_table_iter_next(&iter, NULL, &value)) {
			...
			json_object_set_new(ml, "type", json_string(mp->pullstream_type == janus_pullstream_type_live ? "live" : "on demand"));
			if(mp->pullstream_source == janus_pullstream_source_rtp) {
				janus_pullstream_rtp_source *source = mp->source;
				gint64 now = janus_get_monotonic_time();
		...
		janus_mutex_lock(&mountpoints_mutex);
		janus_pullstream_mountpoint *mp = g_hash_table_lookup(mountpoints, &id_value);
		if(mp == NULL) {
			janus_mutex_unlock(&mountpoints_mutex);
			JANUS_LOG(LOG_VERB, "No such mountpoint/stream %"SCNu64"\n", id_value);
			error_code = JANUS_PULLSTREAM_ERROR_NO_SUCH_MOUNTPOINT;
		...
		json_object_set_new(ml, "type", json_string(mp->pullstream_type == janus_pullstream_type_live ? "live" : "on demand"));
		if(mp->pullstream_source == janus_pullstream_source_file) {
			janus_pullstream_file_source *source = mp->source;
			if(admin && source->filename)
				json_object_set_new(ml, "filename", json_string(source->filename));
		} else if(mp->pullstream_source == janus_pullstream_source_rtp) {
			janus_pullstream_rtp_source *source = mp->source;
			if(source->is_srtp) {
		...
		else if (mp->pullstream_source == janus_pullstream_source_rtmp) {
			janus_pullstream_rtmp_source *source = mp->source;
			if (source->is_srtp) {
				json_object_set_new(ml, "srtp", json_true());
			}
		...
		JANUS_LOG(LOG_INFO, "janus_pullstream_handle_message  create session+++++++++\n");
		...
		if (error_code != 0)
		{
			JANUS_LOG(LOG_INFO, "janus_pullstream_handle_message  error_code +++++++++\n");
			goto plugin_response;
		}
		if(admin_key != NULL) {
			...
			JANUS_CHECK_SECRET(admin_key, root, "admin_key", error_code, error_cause,
				JANUS_PULLSTREAM_ERROR_MISSING_ELEMENT, JANUS_PULLSTREAM_ERROR_INVALID_ELEMENT, JANUS_PULLSTREAM_ERROR_UNAUTHORIZED);
			if(error_code != 0)
				goto plugin_response;
		}
		...
		if(!strcasecmp(type_text, "rtp")) {
			...
			if(!doaudio && !dovideo && !dodata) {
				...
				error_code = JANUS_PULLSTREAM_ERROR_CANT_CREATE;
			...
			mp = janus_pullstream_create_rtp_source(
			...
			/* TODO We should support something more than raw a-Law and mu-Law streams... */
			if(!doaudio || dovideo) {
				JANUS_LOG(LOG_ERR, "Can't add 'live' stream, we only support audio file pullstream right now...\n");
				error_code = JANUS_PULLSTREAM_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "Can't add 'live' stream, we only support audio file pullstream right now...");
				goto plugin_response;
			}
			...
			mp = janus_pullstream_create_file_source(
					id ? json_integer_value(id) : 0,
					name ? (char *)json_string_value(name) : NULL,
					desc ? (char *)json_string_value(desc) : NULL,
					filename,
			...
			/* TODO We should support something more than raw a-Law and mu-Law streams... */
			if(!doaudio || dovideo) {
				JANUS_LOG(LOG_ERR, "Can't add 'ondemand' stream, we only support audio file pullstream right now...\n");
				error_code = JANUS_PULLSTREAM_ERROR_CANT_CREATE;
				g_snprintf(error_cause, 512, "Can't add 'ondemand' stream, we only support audio file pullstream right now...");
				goto plugin_response;
			}
			char *filename = (char *)json_string_value(file);
			if(!strstr(filename, ".alaw") && !strstr(filename, ".mulaw")) {
			...
			error_code = JANUS_PULLSTREAM_ERROR_INVALID_ELEMENT;
			...
			mp = janus_pullstream_create_rtsp_source(
					id ? json_integer_value(id) : 0,
					name ? (char *)json_string_value(name) : NULL,
					desc ? (char *)json_string_value(desc) : NULL,
		...
		if(save) {
			...
			if(janus_config_save(config, config_folder, JANUS_PULLSTREAM_PACKAGE) < 0)
		...
		if(notify_events && gateway->events_is_enabled()) {
			...
			json_object_set_new(info, "type", json_string(mp->pullstream_type == janus_pullstream_type_live ? "live" : "on demand"));
			gateway->notify_event(&janus_pullstream_plugin, session->handle, info);
		}
		goto plugin_response;
	} 
	else if(!strcasecmp(request_text, "edit")) {
		JANUS_LOG(LOG_VERB, "Attempt to edit an existing pullstream mountpoint\n");
		...
		if(save) {
			...
			if(mp->pullstream_source == janus_pullstream_source_rtp) {
				...
				janus_config_add_item(config, mp->name, "type", (mp->pullstream_type == janus_pullstream_type_live) ? "live" : "ondemand");
				janus_pullstream_file_source *source = mp->source;
				janus_config_add_item(config, mp->name, "filename", source->filename);
				janus_config_add_item(config, mp->name, "audio", mp->codecs.audio_pt ? "yes" : "no");
				janus_config_add_item(config, mp->name, "video", mp->codecs.video_pt ? "yes" : "no");
		...
		janus_refcount_decrease(&mp->ref);
		/* Done */
		JANUS_LOG(LOG_VERB, "pullstream mountpoint edited\n");
		goto plugin_response;
	} 
		...
		while(viewer) {
			janus_pullstream_session *session = (janus_pullstream_session *)viewer->data;
			if(session != NULL) {
				...
				gateway->push_event(session->handle, &janus_pullstream_plugin, NULL, event, NULL);
		...
		janus_refcount_increase(&mp->ref);
		if(mp->pullstream_type != janus_pullstream_type_live || mp->pullstream_source != janus_pullstream_source_rtp) {
			janus_refcount_decrease(&mp->ref);
			janus_mutex_unlock(&mountpoints_mutex);
			JANUS_LOG(LOG_ERR, "Recording is only available on RTP-based live streams\n");
			error_code = JANUS_PULLSTREAM_ERROR_INVALID_REQUEST;
		...
		if(!strcasecmp(action_text, "start")) {
			...
			if(audio) {
				...
				if(arc == NULL) {
					...
					janus_refcount_decrease(&mp->ref);
					janus_mutex_unlock(&mountpoints_mutex);
					error_code = JANUS_PULLSTREAM_ERROR_CANT_RECORD;
	...
	else if(!strcasecmp(request_text, "enable") || !strcasecmp(request_text, "disable")) 
	{
		...
		janus_pullstream_message *msg = g_malloc(sizeof(janus_pullstream_message));
		msg->handle = handle;
...
void janus_pullstream_setup_media(janus_plugin_session *handle) {
	JANUS_LOG(LOG_INFO, "[%s-%p] WebRTC media is now available\n", JANUS_PULLSTREAM_PACKAGE, handle);
	if(g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized))
		return;
	janus_mutex_lock(&sessions_mutex);
	janus_pullstream_session *session = janus_pullstream_lookup_session(handle);
	...
	janus_mutex_unlock(&sessions_mutex);
	g_atomic_int_set(&session->hangingup, 0);
	/* We only start pullstream towards this user when we get this event */
	janus_rtp_switching_context_reset(&session->context);
	/* If this is related to a live RTP mountpoint, any keyframe we can shoot already? */
	janus_pullstream_mountpoint *mountpoint = session->mountpoint;
	if(mountpoint->pullstream_source == janus_pullstream_source_rtp) {
		janus_pullstream_rtp_source *source = mountpoint->source;
		if(source->keyframe.enabled) {
			...
			if(source->keyframe.latest_keyframe != NULL) {
				...
				while(temp) {
					janus_pullstream_relay_rtp_packet(session, temp->data);
					temp = temp->next;
				}
			}
		...
		if(source->buffermsg) {
			...
			if(source->last_msg != NULL) {
				...
				janus_pullstream_relay_rtp_packet(session, source->last_msg);
			}
	...
	json_object_set_new(event, "result", result);
	int ret = gateway->push_event(handle, &janus_pullstream_plugin, NULL, event, NULL);
...
void janus_pullstream_incoming_rtcp(janus_plugin_session *handle, int video, char *buf, int len) {
	if(handle == NULL || g_atomic_int_get(&handle->stopped) || g_atomic_int_get(&stopping) || !g_atomic_int_get(&initialized))
		return;
	janus_pullstream_session *session = (janus_pullstream_session *)handle->plugin_handle;
	if(!session || g_atomic_int_get(&session->destroyed) || session->stopping || !session->started || session->paused)
		return;
	janus_pullstream_mountpoint *mp = (janus_pullstream_mountpoint *)session->mountpoint;
	if((mp->pullstream_source != janus_pullstream_source_rtp)&&(mp->pullstream_source != janus_pullstream_source_rtmp))
		return;
	janus_pullstream_rtp_source *source = (janus_pullstream_rtp_source *)mp->source;
	if(!video && (source->audio_rtcp_fd > -1) && (source->audio_rtcp_addr.sa_family != 0)) {
		...
		if(janus_rtcp_has_fir(buf, len) || janus_rtcp_has_pli(buf, len)) {
			...
			janus_pullstream_rtcp_pli_send(source);
		}
...
static void *janus_pullstream_handler(void *data) {
	JANUS_LOG(LOG_VERB, "Joining pullstream handler thread\n");
	janus_pullstream_message *msg = NULL;
	int error_code = 0;
	char error_cause[512];
	json_t *root = NULL;
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(msg == &exit_message)
			break;
		if(msg->handle == NULL) {
			janus_pullstream_message_free(msg);
			continue;
		}
		janus_mutex_lock(&sessions_mutex);
		janus_pullstream_session *session = janus_pullstream_lookup_session(msg->handle);
		if(!session) {
			janus_mutex_unlock(&sessions_mutex);
			JANUS_LOG(LOG_ERR, "No session associated with this handle...\n");
			janus_pullstream_message_free(msg);
		...
		if(!strcasecmp(request_text, "watch")) {
			...
			/*json_t *rtmp_listener = json_object_get(root, "rtmp");
			if (NULL == rtmp_listener) {
				goto error;
			}

			const char *rtmp_listener_text = json_string_value(rtmp_listener);
			if (NULL == strstr(rtmp_listener_text, "rtmp://") || strlen(rtmp_listener_text) <= strlen("rtmp://pull-meet.yflive.net/*")) {
				goto error;
			}*/
			...
			if (id_value == 0) {
				...
				while (id_value == 0) {
					static testid = 10;
					//id_value = janus_random_uint64();
					id_value = testid++;
					if (g_hash_table_lookup(mountpoints, &id_value) != NULL) {
						/* ID already in use, try another one */
						id_value = 0;
					}
				}
			}
			JANUS_LOG(LOG_ERR, " id_value %"PRIu64"\n", id_value);
			janus_pullstream_mountpoint *mp = janus_pullstream_create_rtmp_source(id_value, "rtmp://pull-meet.yflive.net",
				id ? id : "rtmp test 1", "rtmp://127.0.0.1", TRUE, "", "", TRUE, "", "", "", FALSE);
			...
			if(session->mountpoint != NULL) {
				if(session->mountpoint != mp) {
					...
					error_code = JANUS_PULLSTREAM_ERROR_CANT_SWITCH;
			...
			JANUS_LOG(LOG_ERR, "watch finshed session %p  session->mountpoint %d\n", session, session->mountpoint == NULL ? 0 : 1);
			if(mp->pullstream_type == janus_pullstream_type_on_demand) {
				GError *error = NULL;
				char tname[16];
				g_snprintf(tname, sizeof(tname), "mp %"SCNu64, id_value);
				janus_refcount_increase(&session->ref);
				janus_refcount_increase(&mp->ref);
				GThread* mpid_thread = NULL;
				mpid_thread = g_thread_try_new(tname, &janus_pullstream_ondemand_thread, session, &error);
				if(error != NULL) {
					session->mountpoint = NULL;
					janus_mutex_unlock(&mp->mutex);
				...
				janus_pullstream_rtmp_source *source = (janus_pullstream_rtmp_source *)mp->source;
				if (source && source->simulcast) {
					JANUS_VALIDATE_JSON_OBJECT(root, simulcast_parameters,
						error_code, error_cause, TRUE,
						JANUS_PULLSTREAM_ERROR_MISSING_ELEMENT, JANUS_PULLSTREAM_ERROR_INVALID_ELEMENT);
			...
			if(mp->pullstream_source == janus_pullstream_source_rtp) {
				...
				if(mp->helper_threads > 0) {
					int viewers = -1;
					janus_pullstream_helper *helper = NULL;
					GList *l = mp->threads;
					while(l) {
						janus_pullstream_helper *ht = (janus_pullstream_helper *)l->data;
			...
			JANUS_LOG(LOG_ERR, "start session %p  %d\n", session, session->mountpoint == NULL ? 0 : 1);
			if(session->mountpoint == NULL) {
			...
			JANUS_LOG(LOG_VERB, "Starting the pullstream\n");
			session->paused = FALSE;
			result = json_object();
			...
			JANUS_LOG(LOG_VERB, "Pausing the pullstream\n");
			session->paused = TRUE;
			result = json_object();
				...
				if(source && source->svc) {
					...
					if(spatial) {
						...
						if(spatial_layer == session->spatial_layer) {
							...
							json_object_set_new(event, "result", result);
							gateway->push_event(msg->handle, &janus_pullstream_plugin, NULL, event, NULL);
							json_decref(event);
			...
			janus_pullstream_mountpoint *oldmp = session->mountpoint;
			if(oldmp == NULL) {
			...
			if(oldmp->pullstream_type != janus_pullstream_type_live ||
					oldmp->pullstream_source != janus_pullstream_source_rtp) {
			...
			if(mp->pullstream_type != janus_pullstream_type_live ||
					mp->pullstream_source != janus_pullstream_source_rtp) {
				janus_refcount_decrease(&oldmp->ref);
				janus_refcount_decrease(&mp->ref);
				janus_mutex_unlock(&mountpoints_mutex);
			...
			//////////////////////////////////////////////////////////////////////////
			/* 应该在停止的释放mounpoint ，现在客户端没有提供相关的数据*/
			//////////////////////////////////////////////////////////////////////////

			JANUS_LOG(LOG_ERR, "stop session %p  %d\n", session, session->mountpoint == NULL ? 0 : 1);
			if(session->stopping || !session->started) {
				if (session->mountpoint != NULL)
				{
					g_atomic_int_compare_and_exchange(&session->mountpoint->destroyed, 0, 1);
					session->mountpoint = NULL;
					JANUS_LOG(LOG_ERR, "session->mountpoint  !=null  %p\n",session);
				}
				else
				{
					JANUS_LOG(LOG_ERR, "session->mountpoint  ==null %p \n", session);
			...
			JANUS_LOG(LOG_VERB, "Stopping the pullstream\n");
			result = json_object();
			...
			if (session->mountpoint != NULL)
			{
				g_atomic_int_compare_and_exchange(&session->mountpoint->destroyed, 0, 1);
				session->mountpoint = NULL;
				JANUS_LOG(LOG_ERR, " second session->mountpoint  !=null  %p\n", session);
			}
			else
			{
				JANUS_LOG(LOG_ERR, "second session->mountpoint  ==null %p \n", session);
		...
		if(result != NULL)
			json_object_set_new(event, "result", result);
		//json_object_set_new(jsep, "id", id_value);
		int ret = gateway->push_event(msg->handle, &janus_pullstream_plugin, msg->transaction, event, jsep);
		JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (%s)\n", ret, janus_get_api_error(ret));
		g_free(sdp);
		json_decref(event);
		json_decref(jsep);
...
error:
		{
			...
			json_object_set_new(event, "error_code", json_integer(error_code));
			json_object_set_new(event, "error", json_string(error_cause));
			int ret = gateway->push_event(msg->handle, &janus_pullstream_plugin, msg->transaction, event, NULL);
			JANUS_LOG(LOG_VERB, "  >> Pushing event: %d (%s)\n", ret, janus_get_api_error(ret));
			json_decref(event);
			janus_pullstream_message_free(msg);
		}
	}
	JANUS_LOG(LOG_VERB, "Leaving pullstream handler thread\n");
	return NULL;
}

/* Helpers to create a listener filedescriptor */
static int janus_pullstream_create_fd(int port, in_addr_t mcast, const janus_network_address *iface, const char *listenername, const char *medianame, const char *mountpointname) {
...
/* Helpers to destroy a pullstream mountpoint. */
static void janus_pullstream_rtp_source_free(janus_pullstream_rtp_source *source) {
	if(source->audio_fd > -1) {
	...
	janus_mutex_lock(&source->keyframe.mutex);
	if(source->keyframe.latest_keyframe != NULL)
		g_list_free_full(source->keyframe.latest_keyframe, (GDestroyNotify)janus_pullstream_rtp_relay_packet_free);
	source->keyframe.latest_keyframe = NULL;
	janus_mutex_unlock(&source->keyframe.mutex);
	janus_mutex_lock(&source->buffermsg_mutex);
	if(source->last_msg != NULL)
		janus_pullstream_rtp_relay_packet_free((janus_pullstream_rtp_relay_packet *)source->last_msg);
	...
	janus_pullstream_buffer *curldata = source->curldata;
	if(curldata != NULL) {
...
static void janus_pullstream_rtmp_source_free(janus_pullstream_rtp_source *source) {
	if (source->audio_fd > -1) {
		close(source->audio_fd);
	}
	if (source->video_fd[0] > -1) {
...
static void janus_pullstream_file_source_free(janus_pullstream_file_source *source) {
...
janus_pullstream_mountpoint *janus_pullstream_create_rtp_source(
	...
	int audio_rtcp_fd = -1;
	if(doaudio) {
		audio_fd = janus_pullstream_create_fd(aport, amcast ? inet_addr(amcast) : INADDR_ANY, aiface,
		...
		aport = janus_pullstream_get_fd_port(audio_fd);
		if(artcpport > 0) {
			audio_rtcp_fd = janus_pullstream_create_fd(artcpport, amcast ? inet_addr(amcast) : INADDR_ANY, aiface,
				"Audio", "audio", name ? name : tempname);
			if(audio_rtcp_fd < 0) {
	...
	int video_rtcp_fd = -1;
	if(dovideo) {
		video_fd[0] = janus_pullstream_create_fd(vport, vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
		...
		vport = janus_pullstream_get_fd_port(video_fd[0]);
		if(vrtcpport > 0) {
			video_rtcp_fd = janus_pullstream_create_fd(vrtcpport, amcast ? inet_addr(amcast) : INADDR_ANY, aiface,
				"Video", "video", name ? name : tempname);
			if(video_rtcp_fd < 0) {
		...
		if(simulcast) {
			if(vport2 > 0) {
				video_fd[1] = janus_pullstream_create_fd(vport2, vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
					"Video", "video", name ? name : tempname);
				if(video_fd[1] < 0) {
				...
				vport2 = janus_pullstream_get_fd_port(video_fd[1]);
			}
			if(vport3 > 0) {
				video_fd[2] = janus_pullstream_create_fd(vport3, vmcast ? inet_addr(vmcast) : INADDR_ANY, viface,
					"Video", "video", name ? name : tempname);
				if(video_fd[2] < 0) {
				...
				vport3 = janus_pullstream_get_fd_port(video_fd[2]);
			}
		}
	}
	int data_fd = -1;
	if(dodata) {
#ifdef HAVE_SCTP
		data_fd = janus_pullstream_create_fd(dport, INADDR_ANY, diface,
		...
		dport = janus_pullstream_get_fd_port(data_fd);
#else
	...
	janus_pullstream_mountpoint *live_rtp = g_malloc0(sizeof(janus_pullstream_mountpoint));
	live_rtp->id = id;
	...
	live_rtp->pullstream_type = janus_pullstream_type_live;
	live_rtp->pullstream_source = janus_pullstream_source_rtp;
	janus_pullstream_rtp_source *live_rtp_source = g_malloc0(sizeof(janus_pullstream_rtp_source));
	...
	live_rtp->source_destroy = (GDestroyNotify) janus_pullstream_rtp_source_free;
	...
	janus_refcount_init(&live_rtp->ref, janus_pullstream_mountpoint_free);
	...
	if(threads > 0) {
		int i=0;
		for(i=0; i<threads; i++) {
			janus_pullstream_helper *helper = g_malloc0(sizeof(janus_pullstream_helper));
			helper->id = i+1;
			helper->mp = live_rtp;
			helper->queued_packets = g_async_queue_new_full((GDestroyNotify)janus_pullstream_rtp_relay_packet_free);
			...
			helper->thread = g_thread_try_new(tname, &janus_pullstream_helper_thread, helper, &error);
			if(error != NULL) {
				...
				janus_pullstream_mountpoint_destroy(live_rtp);
				return NULL;
			}
	...
	janus_refcount_increase(&live_rtp->ref);
	live_rtp->thread = g_thread_try_new(tname, &janus_pullstream_relay_thread, live_rtp, &error);
	if(error != NULL) {
...
janus_pullstream_mountpoint *janus_pullstream_create_file_source(
	...
	janus_pullstream_mountpoint *file_source = g_malloc0(sizeof(janus_pullstream_mountpoint));
	file_source->id = id;
	char tempname[255];
	if(!name) {
		memset(tempname, 0, 255);
	...
	file_source->pullstream_type = live ? janus_pullstream_type_live : janus_pullstream_type_on_demand;
	file_source->pullstream_source = janus_pullstream_source_file;
	janus_pullstream_file_source *file_source_source = g_malloc0(sizeof(janus_pullstream_file_source));
	file_source_source->filename = g_strdup(filename);
	file_source->source = file_source_source;
	file_source->source_destroy = (GDestroyNotify) janus_pullstream_file_source_free;
	...
	janus_refcount_init(&file_source->ref, janus_pullstream_mountpoint_free);
	...
	if(live) {
		...
		file_source->thread = g_thread_try_new(tname, &janus_pullstream_filesource_thread, file_source, &error);
		if(error != NULL) {
...
#ifdef HAVE_LIBCURL
static size_t janus_pullstream_rtsp_curl_callback(void *payload, size_t size, size_t nmemb, void *data) {
	size_t realsize = size * nmemb;
	janus_pullstream_buffer *buf = (struct janus_pullstream_buffer *)data;
...
static int janus_pullstream_rtsp_parse_sdp(const char *buffer, const char *name, const char *media, int *pt,
	...
	do {
		fds->fd = janus_pullstream_create_fd(0, mcast, iface, media, media, name);
		if(fds->fd < 0) {
			return -1;
		}
		...
		fds->rtcp_fd = janus_pullstream_create_fd(port+1, mcast, iface, media, media, name);
		if(fds->rtcp_fd < 0) {
			close(fds->fd);
			port = -1;
...
static int janus_pullstream_rtsp_connect_to_server(janus_pullstream_mountpoint *mp) {
	if (mp == NULL)
		return -1;
	janus_pullstream_rtp_source *source = (janus_pullstream_rtp_source *)mp->source;
	if (source == NULL)
	...
	janus_pullstream_buffer *curldata = g_malloc(sizeof(janus_pullstream_buffer));
	...
	curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, janus_pullstream_rtsp_curl_callback);
	curl_easy_setopt(curl, CURLOPT_WRITEDATA, curldata);
	curl_easy_setopt(curl, CURLOPT_HEADERFUNCTION, janus_pullstream_rtsp_curl_callback);
	...
	int vresult;
	vresult = janus_pullstream_rtsp_parse_sdp(curldata->buffer, name, "video", &vpt,
		vtransport, vhost, vrtpmap, vfmtp, vcontrol, &source->video_iface, &video_fds);
	vpt = 126;
	int aresult;
	aresult = janus_pullstream_rtsp_parse_sdp(curldata->buffer, name, "audio", &apt,
...
static int janus_pullstream_rtmp_connect_to_server(janus_pullstream_mountpoint *mp) {
	if (mp == NULL)
		return -1;
	janus_pullstream_rtp_source *source = (janus_pullstream_rtp_source *)mp->source;
	if (source == NULL)
	...
	gboolean dovideo = mp->video;
#ifdef USE_PCM
	mp->codecs.audio_pt = 8;
	mp->codecs.audio_fmtp = "8";
	mp->codecs.audio_rtpmap = "PCMA/8000";
#else
	mp->codecs.audio_pt = 109;
	mp->codecs.audio_fmtp = "109";
	mp->codecs.audio_rtpmap = "OPUS/48000/2";
#endif
	mp->codecs.video_pt = 126;
	mp->codecs.video_fmtp = "126";
	mp->codecs.video_rtpmap = "H264/90000";
	mp->rtp_muxer = rtp_muxer_init(mp->codecs.video_pt, "h264", rtp_muxer_packet_callback, mp, mp->codecs.audio_pt, "opus", rtp_muxer_packet_callback, mp);
	mp->opus_encoder = pcm_to_opus_encode_init(48000, 2, 0, opus_encoder_callback, mp);
	mp->aac_decoder = aac_decoder_init(2, 48000, mp, aac_decoder_callback);
	mp->flv_demuxer = flv_demuxer_video_audio_init(mp, flv_demuxer_video_audio_callback);
	mp->rtmp_client = rtmp_client_live_play_init("192.168.1.244", "pull-meet.yflive.net/h5live", "/aaa", 1935, 2000, mp, rtmp_client_live_play_callback);
	return 0;
}

/* Helper method to send a latching packet on an RTSP media socket */
static void janus_pullstream_rtsp_latch(int fd, char *host, int port, struct sockaddr *remote) {
...
static int janus_pullstream_rtsp_play(janus_pullstream_rtp_source *source) {
	...
	if(source->remote_audio_port > 0 && source->audio_fd >= 0) {
		...
		janus_pullstream_rtsp_latch(source->audio_fd, source->rtsp_ahost, source->remote_audio_port, &remote);
		if(source->remote_audio_rtcp_port > 0 && source->audio_rtcp_fd >= 0) {
			...
			janus_pullstream_rtsp_latch(source->audio_rtcp_fd, source->rtsp_ahost,
	...
	if(source->remote_video_port > 0 && source->video_fd[0] >= 0) {
		...
		janus_pullstream_rtsp_latch(source->video_fd[0], source->rtsp_vhost, source->remote_video_port, &remote);
		if(source->remote_video_rtcp_port > 0 && source->video_rtcp_fd >= 0) {
			...
			janus_pullstream_rtsp_latch(source->video_rtcp_fd, source->rtsp_vhost,
...
janus_pullstream_mountpoint *janus_pullstream_create_rtsp_source(
	...
	JANUS_LOG(LOG_INFO, "janus_pullstream_create_rtsp_source pullstream +++++++++++++++++\n");
	if (url == NULL) {
	...
	janus_pullstream_mountpoint *live_rtsp = g_malloc0(sizeof(janus_pullstream_mountpoint));
	live_rtsp->id = id;
	...
	live_rtsp->pullstream_type = janus_pullstream_type_live;
	live_rtsp->pullstream_source = janus_pullstream_source_rtp;
	janus_pullstream_rtp_source *live_rtsp_source = g_malloc0(sizeof(janus_pullstream_rtp_source));
	...
	live_rtsp->source_destroy = (GDestroyNotify)janus_pullstream_rtp_source_free;
	live_rtsp->viewers = NULL;
	g_atomic_int_set(&live_rtsp->destroyed, 0);
	janus_refcount_init(&live_rtsp->ref, janus_pullstream_mountpoint_free);
	...
	if(error_on_failure) {
		...
		if(janus_pullstream_rtsp_connect_to_server(live_rtsp) < 0) {
		...
		if(janus_pullstream_rtsp_play(live_rtsp_source) < 0) {
	...
	live_rtsp->thread = g_thread_try_new(tname, &janus_pullstream_relay_thread, live_rtsp, &error);
	if (error != NULL) {
	...
	gboolean error_on_failure)
{
	...
	JANUS_LOG(LOG_INFO, "janus_pullstream_create_rtmp_source pullstream +++++++++++++++++\n");
	if (url == NULL) {
		JANUS_LOG(LOG_ERR, "Can't add 'rtmp' stream, missing url...\n");
		return NULL;
	}
	JANUS_LOG(LOG_VERB, "Audio %s, Video %s\n", doaudio ? "enabled" : "NOT enabled", dovideo ? "enabled" : "NOT enabled");
	/* Create an RTP source for the media we'll get */
	...
	live_rtsp->data = FALSE;
	live_rtsp->pullstream_type = janus_pullstream_type_live;
	live_rtsp->pullstream_source = janus_pullstream_source_rtmp;
	janus_pullstream_rtmp_source *live_rtsp_source = g_malloc0(sizeof(janus_pullstream_rtmp_source));
	live_rtsp_source->rtsp = TRUE;
	live_rtsp_source->rtsp_url = g_strdup(url);
	live_rtsp_source->arc = NULL;
	live_rtsp_source->vrc = NULL;
	...
	janus_mutex_init(&live_rtsp_source->rtsp_mutex);
	live_rtsp->source = live_rtsp_source;
	live_rtsp->source_destroy = (GDestroyNotify)janus_pullstream_rtmp_source_free;
	live_rtsp->viewers = NULL;
	g_atomic_int_set(&live_rtsp->destroyed, 0);
	janus_refcount_init(&live_rtsp->ref, janus_pullstream_mountpoint_free);
	janus_mutex_init(&live_rtsp->mutex);
	...
	/* Now connect to the RTMP server */
	if (janus_pullstream_rtmp_connect_to_server(live_rtsp) < 0) {
		/* Error connecting, get rid of the mountpoint */
		janus_refcount_decrease(&live_rtsp->ref);
		return NULL;
	}
	...
	g_snprintf(tname, sizeof(tname), "mp %"SCNu64, live_rtsp->id);
	janus_refcount_increase(&live_rtsp->ref);
	live_rtsp->thread = g_thread_try_new(tname, &janus_pullstream_rtmpsource_thread, live_rtsp, &error);
	if (error != NULL) {
		JANUS_LOG(LOG_ERR, "Got error %d (%s) trying to launch the RTSP thread...\n", error->code, error->message ? error->message : "??");
		janus_refcount_decrease(&live_rtsp->ref);	/* This is for the failed thread */
		janus_refcount_decrease(&live_rtsp->ref);
...
static void *janus_pullstream_ondemand_thread(void *data) {
	...
	janus_pullstream_session *session = (janus_pullstream_session *)data;
	if(!session) {
	...
	if(mountpoint->pullstream_source != janus_pullstream_source_file) {
	...
	if(mountpoint->pullstream_type != janus_pullstream_type_on_demand) {
	...
	janus_pullstream_file_source *source = mountpoint->source;
	...
	/* Loop */
	gint read = 0;
	janus_pullstream_rtp_relay_packet packet;
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&mountpoint->destroyed) && !session->stopping && !g_atomic_int_get(&session->destroyed)) {
		...
		/* Go! */
		janus_pullstream_relay_rtp_packet(session, &packet);
		/* Update header */
		seq++;
		header->seq_number = htons(seq);
		ts += 160;
...
static void *janus_pullstream_filesource_thread(void *data) {
	...
	janus_pullstream_mountpoint *mountpoint = (janus_pullstream_mountpoint *)data;
	if(!mountpoint) {
		JANUS_LOG(LOG_ERR, "Invalid mountpoint!\n");
		return NULL;
	}
	...
	if(mountpoint->pullstream_type != janus_pullstream_type_live) {
	...
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&mountpoint->destroyed)) {
		...
		g_list_foreach(mountpoint->viewers, janus_pullstream_relay_rtp_packet, &packet);
	...
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&mountpoint->destroyed)) {
		...
		if(source->rtsp) {
			...
			if(!source->reconnecting && (now - source->reconnect_timer > 5*G_USEC_PER_SEC)) {
				...
				if(janus_pullstream_rtsp_connect_to_server(mountpoint) < 0) {
					...
					if(janus_pullstream_rtsp_play(source) < 0) {
		...
		if(source->video_rtcp_fd > -1 && source->lowest_bitrate > 0) {
			...
			if(source->remb_latest == 0)
				...
				janus_pullstream_rtcp_remb_send(source);
		}
		/* Prepare poll */
		num = 0;
		if(audio_fd != -1) {
			...
			if(fds[i].revents & (POLLERR | POLLHUP)) {
				...
				if(pipe_fd != -1 && fds[i].fd == pipe_fd) {
					...
					g_list_foreach(mountpoint->helper_threads == 0 ? mountpoint->viewers : mountpoint->threads,
						mountpoint->helper_threads == 0 ? janus_pullstream_relay_rtp_packet : janus_pullstream_helper_rtprtcp_packet,
						&packet);
					...
					if(source->keyframe.enabled) {
						if(source->keyframe.temp_ts > 0 && ntohl(rtp->timestamp) != source->keyframe.temp_ts) {
							...
							janus_pullstream_rtp_relay_packet *pkt = g_malloc0(sizeof(janus_pullstream_rtp_relay_packet));
					...
					*(text+bytes) = '\0';
					...
					janus_mutex_lock(&mountpoint->mutex);
					g_list_foreach(mountpoint->helper_threads == 0 ? mountpoint->viewers : mountpoint->threads,
						mountpoint->helper_threads == 0 ? janus_pullstream_relay_rtcp_packet : janus_pullstream_helper_rtprtcp_packet,
						&packet);
					janus_mutex_unlock(&mountpoint->mutex);
...
static void *janus_pullstream_rtmpsource_thread(void *data) 
{
	JANUS_LOG(LOG_INFO, "Starting streaming relay thread ++++++++++++++++\n");
	janus_pullstream_mountpoint *mountpoint = (janus_pullstream_mountpoint *)data;
	if (mountpoint->rtmp_client != NULL)
	{
		rtmp_client_live_play_connect(mountpoint->rtmp_client);
	}
	else
	{
		JANUS_LOG(LOG_INFO, "Starting streaming relay thread, but mountpoint->rtmp_client is null");
		return;
	}
	while (!g_atomic_int_get(&mountpoint->destroyed))
	{
		if (rtmp_client_live_play_start(mountpoint->rtmp_client) < 0)
		{
			rtmp_client_live_play_connect(mountpoint->rtmp_client);
		}
	}
	janus_mutex_lock(&mountpoints_mutex);
	JANUS_LOG(LOG_ERR, "mountpoint->id %"PRIu64"\n", mountpoint->id);
	if (g_hash_table_lookup(mountpoints, &mountpoint->id) != NULL) {
		g_hash_table_remove(mountpoints, mountpoint->id);
	}
	janus_mutex_unlock(&mountpoints_mutex);
	janus_pullstream_mountpoint_destroy(mountpoint);
}

static void janus_pullstream_relay_rtp_packet(gpointer data, gpointer user_data) {
	janus_pullstream_rtp_relay_packet *packet = (janus_pullstream_rtp_relay_packet *)user_data;
...
static void janus_pullstream_helper_rtprtcp_packet(gpointer data, gpointer user_data) {
	...
	janus_pullstream_helper *helper = (janus_pullstream_helper *)data;
	if(!helper) {
		//~ JANUS_LOG(LOG_ERR, "Invalid session...\n");
		return;
	}
	/* Clone the packet and queue it for delivery on the helper thread */
	janus_pullstream_rtp_relay_packet *copy = g_malloc0(sizeof(janus_pullstream_rtp_relay_packet));
...
static void *janus_pullstream_helper_thread(void *data) {
	janus_pullstream_helper *helper = (janus_pullstream_helper *)data;
	janus_pullstream_mountpoint *mp = helper->mp;
	JANUS_LOG(LOG_INFO, "[%s/#%d] Joining Streaming helper thread\n", mp->name, helper->id);
	janus_pullstream_rtp_relay_packet *pkt = NULL;
	while(!g_atomic_int_get(&stopping) && !g_atomic_int_get(&mp->destroyed)) {
		...
		g_list_foreach(helper->viewers,
			pkt->is_rtp ? janus_pullstream_relay_rtp_packet : janus_pullstream_relay_rtcp_packet,
			pkt);
		janus_mutex_unlock(&helper->mutex);
		janus_pullstream_rtp_relay_packet_free(pkt);
	}
...
int rtmp_client_live_play_callback(void* param, const unsigned char* data, size_t bytes, uint32_t timestamp, int type)
{
	struct rtmp_client_live_play_context_t * pcontext = (struct rtmp_client_live_play_context_t *)param;
	janus_pullstream_mountpoint * mountpoint = (janus_pullstream_mountpoint*)(pcontext->param);

	flv_demuxer_video_audio_input(mountpoint->flv_demuxer, type, data, bytes, timestamp);
	return 0;
}

#define FLV_AUDIO_OPUS 12356
void flv_demuxer_video_audio_callback(void* param, int codec, const unsigned char* data, size_t bytes, uint32_t pts, uint32_t dts, int flags)
{
	struct flv_demuxer_video_audio_context_t * pcontext = (struct flv_demuxer_video_audio_context_t *)param;
	janus_pullstream_mountpoint * mountpoint = (janus_pullstream_mountpoint*)(pcontext->param);
	if (FLV_AUDIO_AAC == codec || FLV_AUDIO_ASC == codec)
	{
		aac_decoder_input(mountpoint->aac_decoder, data, bytes, pts);
	}
	else if (FLV_VIDEO_H264 == codec || FLV_VIDEO_AVCC == codec || FLV_VIDEO_HVCC == codec)
	{
		rtp_muxer_input(mountpoint->rtp_muxer, data, bytes, pts, RTP_VIDEO);
	}
	else if(FLV_AUDIO_OPUS == codec){
		rtp_muxer_input(mountpoint->rtp_muxer, data, bytes, pts, RTP_AUDIO);
	}
	else {
		assert(0);
	}
...
void aac_decoder_callback(void* param, const unsigned char* pdata, int bytes, uint32_t timestamp)
{
	struct aac_decoder_context_t * pcontext = (struct aac_decoder_context_t *)param;
	janus_pullstream_mountpoint * mountpoint = (janus_pullstream_mountpoint*)(pcontext->param);
#if ((defined  RESAMPLE_PCM) && (defined  USE_PCM))
	int scale = 6;
	int32_t* pcm = (int32_t*)pdata;
	int32_t resample_pcm[512];//= (int*)mountpoint->opus_encoder->convert_buf;
	int sample = 0;
	do
	{
		assert(sample * 4 * scale  < bytes);
		resample_pcm[sample] = pcm[sample*scale];
		sample++;
	} while ( sample* scale  < (bytes /4));
...
#ifdef USE_PCM
	rtp_muxer_input(mountpoint->rtp_muxer, pdata, bytes, pcontext->n * 960, RTP_AUDIO);
#else
	pcm_to_opus_encode_input(mountpoint->opus_encoder, pdata, bytes, pcontext->n * 960);
#endif
	fwrite(pdata, 1, bytes, pcontext->pcm_fd);
	pcontext->n++;
}

void opus_encoder_callback(void* param, const unsigned char* pdata, int bytes, uint32_t timestamp)
{
	struct opus_encode_context_t* pcontext = (struct opus_encode_context_t*)param;
	janus_pullstream_mountpoint * mountpoint = (janus_pullstream_mountpoint*)(pcontext->param);
	rtp_muxer_input(mountpoint->rtp_muxer, pdata, bytes, timestamp, RTP_AUDIO);
}

void rtp_muxer_packet_callback(void* param, const void *packet, int bytes, RTP_TYPE type)
{
	struct rtp_muxer_context_t* pcontext = (struct rtp_muxer_context_t*)param;
	fwrite(packet, bytes, 1, pcontext->rtp_fd);
	rtp_send(pcontext->audio_param, packet, bytes, type);
}
...
static void rtp_send(janus_pullstream_mountpoint* mountpoint, const unsigned char* pdata, int bytes, RTP_TYPE type)
{
	janus_pullstream_rtp_relay_packet packet;
	janus_pullstream_rtp_source*  source = mountpoint->source;
	janus_rtp_header *rtp = (janus_rtp_header *)pdata;
	...
	if (type == RTP_VIDEO)
	{
		...
		packet.svc = FALSE;
		packet.data->type = mountpoint->codecs.video_pt;
		janus_rtp_header_update(packet.data, &source->context[1], TRUE, 0);
		packet.timestamp = ntohl(packet.data->timestamp);
		packet.seq_number = ntohs(packet.data->seq_number);
		janus_mutex_lock(&mountpoint->mutex);
		g_list_foreach(mountpoint->helper_threads == 0 ? mountpoint->viewers : mountpoint->threads,

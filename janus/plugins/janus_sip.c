	...
	while(g_atomic_int_get(&initialized) && !g_atomic_int_get(&stopping)) {
		...
		if(!strcasecmp(request_text, "register")) {
			...
			if(!refresh && session->stack == NULL) {
				...
				janus_refcount_increase(&session->ref);
				GThread* sip_user_thread = NULL;
				sip_user_thread = g_thread_try_new(tname, janus_sip_sofia_thread, session, &error);
				if(error != NULL) {
					janus_refcount_decrease(&session->ref);
			...
			if(answer) {
				...
				janus_refcount_increase(&session->ref);
				GThread* siprtp_user_thread = NULL;
				siprtp_user_thread = g_thread_try_new(tname, janus_sip_relay_thread, session, &error);
				if(error != NULL) {
					janus_refcount_decrease(&session->ref);

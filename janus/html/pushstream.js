			...
			janus = new Janus(
				{
					...
					success: function() {
						...
						janus.attach(
							{
								plugin: "janus.plugin.pushstream",
								opaqueId: opaqueId,
												...
												if(uplink !== 0) {
													...
													recordplay.send({
														'message': {
															...
															'video-keyframe-interval': 2000 // Keep the 15 seconds key frame interval
														}
													});
												}
	...
	bootbox.prompt("Insert a name for the recording (e.g., John Smith says hello)", function(result) {
		...
		recordplay.send({
			'message': {
				...
				'video-keyframe-interval': 2000 // 15 seconds
			}
		});
		...
		recordplay.createOffer(
			{
				...
				success: function(jsep) {
					Janus.debug("Got SDP!");
					Janus.debug(jsep);
					var body = { "request": "record", "name": myname ,"rtmp": "rtmp://push-meet.yflive.net/h5live/"};
					recordplay.send({"message": body, "jsep": jsep});
				},
				error: function(error) {
					Janus.error("WebRTC error...", error);

...
int janus_process_incoming_request(janus_request *request) {
	...
	if(!strcasecmp(message_text, "keepalive")) {
		...
		if (auth_url != NULL)
		{
			JANUS_LOG(LOG_WARN, "[attach]before auth auth_url : %s \n", auth_url);
		}else{
			JANUS_LOG(LOG_WARN, "[attach]before auth auth_url : is NULL");
		}
		
		if(janus_auth_is_enabled()) {
			//--do something for personal auth
			json_t *user = json_object_get(root, "user");
			json_t *sign = json_object_get(root, "sign");
			if (user != NULL && sign != NULL) {
				const char *user_value = json_string_value(user);
				const char *sign_value = json_string_value(sign);
				if (!janus_auth_personal_auth(auth_url, user_value, sign_value, plugin_t)) {
					JANUS_LOG(LOG_WARN, "user_value '%s' sign_value '%s' can't access plugin '%s'\n", user_value, sign_value, plugin_text);
				...
				JANUS_LOG(LOG_WARN, "user or sign is NULL  can't access plugin '%s'\n", plugin_text);
				ret = janus_process_error(request, session_id, transaction_text, JANUS_ERROR_UNAUTHORIZED_PLUGIN, "Provided token can't access plugin '%s'", plugin_text);
				goto jsondone;
			}
			//--
...
gint main(int argc, char *argv[])
{
	...
	/* auth_url*/
	item = janus_config_get_item_drilldown(config, "general", "auth_url");
	if (item && item->value) {
		auth_url = g_strdup(item->value);
	}
	...
	dir = opendir(path);
	if(!dir) {
		JANUS_LOG(LOG_FATAL, "\tCouldn't access plugins folder... ,errno %s\n", strerror(errno));
		exit(1);
	}
